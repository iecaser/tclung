...
for img_file in file_list:
    ...
    # out_nodemasks.append(node_masks)
...
# final_images = np.ndarray([num_images, 3, 512, 512], dtype=np.float32)
# final_masks = np.ndarray([num_images, 3, 512, 512], dtype=np.float32)
for i in range(num_images):
    # [i,0]这个0是因为多了一维,是为了卷积层的统一（后面将有多个卷积核）
    try:
        # final_images[i] = out_images[i]
        # final_masks[i] = out_nodemasks[i]
